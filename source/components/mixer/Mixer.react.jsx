...
class Mixer extends React.Component {
	...
	getLeftTrackImg() {
		if (this.props.leftTrack) {
			return (
				<img className='mixer-img' src={this.props.leftTrack.thumbnail}></img>
			);
		} else {
			return (
				<img className='mixer-img' src='icons/mipmap-xxxhdpi/ic_launcher.png'></img>
			);
		}
	}
	...
	getRightTrackImg() {
		if (this.props.rightTrack) {
			return (
				<img className='mixer-img' style={{
					float: 'right'
				}} src={this.props.rightTrack.thumbnail}></img>
			);
		} else {
			return (
				<img className='mixer-img' style={{
					float: 'right'
				}} src='icons/mipmap-xxxhdpi/ic_launcher.png'></img>
			);
		}
	}
	...
	buildList() {
		...
		for (var k in this.props.tracks) {
			if (this.props.tracks[k].ready && !this.props.tracks[k].removed) {
				arr.push(<Selector key={this.props.tracks[k].id} track={this.props.tracks[k]} onSelect={this.selectTrack} currentSelector={this.state.currentSelector}/>)
			}
		}
		return arr;
	}
	...
	render() {
		return (
			<div className='mixer'>
				<div style={this.state.toggleStyle} className='mixer-select'>
					{this.buildList()}
				</div>
				<div className='mixer-ctrl'>
					<a onClick={this.selectLeft} style={{
						marginLeft: '0px',
					...
					<TouchSlider handleClassName='mixer-handle' min={0} max={100} value={this.props.crossfadeValue} onChange={this.crossfade} className='mixer-bar'/>
					<a onClick={this.selectRight} style={{
						marginRight: '0px',
